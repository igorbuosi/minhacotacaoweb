// Função para criar mascara do cepa
function criaMascaraCEP(nomecampo='') {
    console.log('Teste: '+nomecampo);
    const maximoInput = document.getElementById(nomecampo).maxLength;
    let valorInput = document.getElementById(nomecampo).value;
    let valorSemPonto = document.getElementById(nomecampo).value.replace(/([^0-9])+/g, "");
    const mascaras = valorInput.replace(/[^\d]/g, "").replace(/(\d{5})(\d{3})/, "$1-$2");

    valorInput.length === maximoInput ? document.getElementById(nomecampo).value = mascaras
            : document.getElementById(nomecampo).value = valorSemPonto;
}

//-------------TRATAMENTO CAMPO cnpj--------------------------
...
$(document).ready(function () {
    ...
    $('#cnpj').blur(function () {
        console.log("testandooooooooooooo");

        var cpfCnpjLimpo = $('#cnpj').unmask().val();
        console.log("CPF/CNPJ Limpo:");
        console.log(cpfCnpjLimpo);

        if (!validarCpfCnpj(cpfCnpjLimpo)) {
            document.getElementById('labelcpfcnpj').innerHTML = "<FONT COLOR='red'>CPF/CNPJ inválido</FONT>";
            document.getElementById('cnpj').classList.remove("is-valid");
            document.getElementById('cnpj').classList.add("is-invalid");
        } else {
            //carregarPessoa($('#cnpj').unmask().val());
            trocaMascaraCpfCnpj($('#cnpj').val());
...
function criaMascaraTelefone(nomecampo="") {
    const maximoInput = document.getElementById(nomecampo).maxLength;
    let valorInput = document.getElementById(nomecampo).value;
    let valorSemPonto = document.getElementById(nomecampo).value.replace(/([^0-9])+/g, "");
